...
gen ingper=inghog1d/(mieperho*12) //ingreso percapita mensual
...
drop if p301a==.
*drop if p301a==12 //opcional
keep if p204==1

*Años de educación (escolaridad)
fre p301a
fre p301b
fre p301c
...
*          Cap400
*--------------------------- 
use enaho01a-2021-400, clear

keep if p204==1

gen disca=(p401h1==1 | p401h2==1 | p401h3==1 | p401h4==1 | p401h5==1 | p401h6==1)

fre disca

lab variable disca "Persona con alguan discapacidad"

lab def disca 0 "No tiene discapacidad" 1 "Tiene alguna discapacidad"
lab value disca disca

keep aÑo conglome vivienda hogar codperso disca
...
merge m:1 conglome vivienda hogar using cap100
drop _merge

merge m:1 conglome vivienda hogar using sumaria
drop _merge
...
merge 1:1 conglome vivienda hogar codperso using cap400
keep if _merge==3
drop _merge
...
save base_enaho, replace //a nivel de personas

duplicates drop aÑo conglome vivienda hogar, force

save base_enaho_hogares, replace
